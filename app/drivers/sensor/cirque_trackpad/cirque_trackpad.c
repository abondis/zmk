...
static int pinnacle_channel_get(
...
static int pinnacle_sample_fetch(
  const struct device *dev, enum sensor_channel chan) {
    uint8_t packet[3];
    ...
    // LOG_WRN("read sample, x: %d, y: %d", packet[1], packet[2]);
...
static void pinnacle_work_cb(struct k_work *work) {
    ...
    pinnacle_write(
      data->dev, PINNACLE_STATUS1, 0);  // Clear SW_DR
}
#endif

static void pinnacle_gpio_cb(
  const struct device *port, struct gpio_callback *cb, uint32_t pins) {
    struct pinnacle_data *data = CONTAINER_OF(
      cb, struct pinnacle_data, gpio_cb);
...
static int pinnacle_init(const struct device *dev) {
    ...
    k_thread_create(
      &data->thread, data->thread_stack, CONFIG_PINNACLE_THREAD_STACK_SIZE,
